...
async function fetchGoogleFlights({ origin, destination, departDate, returnDate, adults, children, tripType }) {
  ...
  const params = new URLSearchParams({ engine: "google_flights", departure_id: origin, arrival_id: destination, outbound_date: departDate, currency: "USD", hl: "en", adults: String(adults), children: String(children), type: tripType === "return" ? "1" : "2", api_key: process.env.SERPAPI_KEY });
  if (tripType === "return" && returnDate) params.set("return_date", returnDate);
  const response = await fetch(`https://serpapi.com/search.json?${params.toString()}`);
  const data = await response.json();
  if (!response.ok || data.error) throw new Error(data.error || `SerpApi returned ${response.status}`);
  const flights = [...(data.best_flights || []), ...(data.other_flights || [])];
  console.log(`SerpApi returned ${flights.length} flights`);
  if (!flights.length) return getMockFlights(origin, destination, departDate);
