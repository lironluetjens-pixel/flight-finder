...
export default function SearchForm({ onSearch, loading }) {
  ...
  const submit=(e)=>{e.preventDefault();if(!origin||origin.length!==3)return alert("Please select a valid departure airport");if(!destination||destination.length!==3)return alert("Please select a valid destination airport");if(!departDate)return alert("Please pick a departure date");if(tripType==="return"&&!returnDate)return alert("Please pick a return date");onSearch({origin,destination,departDate,returnDate,adults,tripType,children:childAges.length,childAges})}
  const totalPax=parseInt(adults||0)+childAges.length
    ...
    <div style={{background:"var(--navy)",borderRadius:"var(--radius)",padding:"40px 48px",marginBottom:32,position:"relative",overflow:"hidden"}}>
      ...
      <form onSubmit={submit}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          <AirportInput value={origin} onChange={setOrigin} label="From" placeholder="City or airport code"/>
          <AirportInput value={destination} onChange={setDestination} label="To" placeholder="City or airport code"/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:tripType==="return"?"1fr 1fr":"1fr",gap:12,marginBottom:4}}>
          <div>
        ...
        <div style={{marginBottom:16,marginTop:8}}>
          ...
          <input style={{...inp,width:"50%"}} type="number" min={1} max={9} value={adults} onChange={e=>setAdults(e.target.value)}/>
        </div>
        ...
        {totalPax>0&&<div style={{fontSize:13,color:"rgba(255,255,255,0.4)",marginBottom:20}}>{adults} adult{adults!=1?"s":""}{childAges.length>0?` + ${childAges.length} child${childAges.length!==1?"ren":""} (ages ${childAges.join(", ")})`:""} — {totalPax} passengers total</div>}
