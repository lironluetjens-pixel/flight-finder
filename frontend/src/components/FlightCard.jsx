...
const labelColors = { Book: { bg: "#dcfce7", text: "#166534", border: "#bbf7d0" }, Wait: { bg: "#fef9c3", text: "#854d0e", border: "#fef08a" }, Ignore: { bg: "#fee2e2", text: "#991b1b", border: "#fecaca" } }
...
export default function FlightCard({ flight: f, rank }) {
  ...
  return (
    <div style={{ background:"#fff", borderRadius:14, marginBottom:14, border:`1px solid ${f.label==="Book"?"#bbf7d0":"#e8e8e4"}`, boxShadow:f.label==="Book"?"0 2px 12px rgba(22,163,74,0.08)":"none", overflow:"hidden" }}>
      <div style={{ padding:"20px 24px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
            <div style={{ fontSize:12, color:"#888" }}>{f.stops===0?"Direct":`${f.stops} stop${f.stops>1?"s":""}${f.stopCity?` via ${f.stopCity}`:""}`}</div>
          ...
          <div style={{ textAlign:"right", minWidth:90 }}>
            ...
            {f.pricePerChild && f.pricePerChild < f.pricePerAdult && <div style={{ fontSize:12, color:"#16a34a", marginTop:2 }}>${Math.round(f.pricePerChild)} child</div>}
          ...
          <div style={{ fontSize:11, color:"#ccc", minWidth:20 }}>#{rank}</div>
        </div>
        <div style={{ marginTop:14, paddingTop:12, borderTop:"1px solid #f0f0ec", display:"flex", flexWrap:"wrap", gap:12 }}>
          <span style={{ fontSize:13, color:"#555" }}>{f.baggageIncluded?"?":"??"} {f.baggageDetails}</span>
          {f.seatsTogether===true && <span style={{ fontSize:13, color:"#16a34a" }}>? Family seating guaranteed</span>}
          {f.seatsTogether===false && <span style={{ fontSize:13, color:"#dc2626" }}>?? Must pay to sit together</span>}
          {f.seatsAvailable && <span style={{ fontSize:13, color:f.seatsAvailable<=3?"#dc2626":"#888" }}>{f.seatsAvailable<=3?`?? Only ${f.seatsAvailable} seats left`:`${f.seatsAvailable} seats available`}</span>}
        </div>
        {f.reasons?.length > 0 && <button onClick={()=>setOpen(o=>!o)} style={{ marginTop:10, fontSize:13, color:"#2563eb", background:"none", border:"none", cursor:"pointer", padding:0 }}>{open?"? Hide score breakdown":"? Why this score?"}</button>}
      </div>
      {open && f.reasons && (
        <div style={{ background:"#f8f8f6", borderTop:"1px solid #eee", padding:"12px 24px" }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {f.reasons.map((r,i) => <span key={i} style={{ fontSize:12, padding:"3px 10px", borderRadius:20, background:"#fff", border:"1px solid #e0e0e0", color:"#444" }}>{r}</span>)}
          </div>
        </div>
      )}
    </div>
