...
const fmt=(iso)=>{ try{ const d=new Date(iso); if(!iso||isNaN(d)) return null; return d.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"}) }catch{return null} }
...
function BookingLinks({f}){
  ...
  const links=[
    ...
    {label:"Skyscanner",color:"#0770e3",url:`https://www.skyscanner.net/transport/flights/${(f.origin||"").toLowerCase()}/${(f.destination||"").toLowerCase()}/${ds}`},
    {label:"Kayak",color:"#ff690f",url:`https://www.kayak.com/flights/${f.origin}-${f.destination}/${date}`},
  ]
  return(
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
      {links.map(l=><a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer" style={{padding:"7px 14px",fontSize:12,fontWeight:600,background:l.color,color:"#fff",borderRadius:7,textDecoration:"none"}} onMouseOver={e=>e.currentTarget.style.opacity="0.8"} onMouseOut={e=>e.currentTarget.style.opacity="1"}>{l.label}</a>)}
    </div>
  )
}
export default function FlightCard({flight:f,rank,animClass}){
  ...
  const depTime=fmt(f.departureTime)
  const arrTime=fmt(f.arrivalTime)
  const hasChildren=f.pricePerChild&&f.pricePerChild<f.pricePerAdult
  return(
        ...
        <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",gap:12,minWidth:260}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:26,fontWeight:700,letterSpacing:-0.5,color:depTime?"var(--text)":"var(--muted)"}}>{depTime||"--:--"}</div>
            ...
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:26,fontWeight:700,letterSpacing:-0.5,color:arrTime?"var(--text)":"var(--muted)"}}>{arrTime||"--:--"}</div>
          ...
          <div style={{textAlign:"right",minWidth:110}}>
            ...
            {hasChildren&&<div style={{fontSize:12,color:"var(--green)",marginTop:2,fontWeight:500}}>${Math.round(f.pricePerChild)} per child</div>}
