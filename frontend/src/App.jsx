    ...
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "40px 20px 80px" }}>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
          {["Direct flights prioritised","Baggage scoring","Family seating check","Book on 3 sites"].map(label=>(
            <div key={label} style={{ fontSize: 12, color: "var(--muted)", padding: "4px 12px", border: "1px solid var(--border)", borderRadius: 100, background: "var(--white)" }}>{label}</div>
          ))}
        </div>
      </div>
      ...
      {results && (
        <div className="fade-up">
          ...
          {results.length===0
            ?<div style={{ textAlign: "center", padding: 60, color: "var(--muted)" }}>No flights found for this route and date.</div>
          ...
          <div style={{ marginTop: 32, padding: "16px 20px", background: "var(--white)", borderRadius: 12, border: "1px solid var(--border)", fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>
            Prices sourced from Google Flights and may change. Click a booking link to complete your purchase directly. Family Wings does not sell tickets.
          </div>
        </div>
      )}
